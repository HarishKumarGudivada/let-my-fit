...
public class ProductServiceImpl implements ProductServcie {
	...
	public List<Order> loadAllOrders() {
		...
		try {
			data=productRepository.findByOrderStatus(new Integer(1));
		}catch (Exception e) {
			e.printStackTrace();
		}
		return data;
	...
	@Override
	public void updateStatusAndAddPrice(int orderId, int status, double addPrice) {
		Order order=new Order();
		order.setAddPrice(addPrice);
